...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<Order>> createOrder(@Valid @RequestBody OrderRequestDTO orderRequestDTO) {
        ...
        ApiResponse<Order> response = new ApiResponse<>(AppUtil.SUCCESS, HttpStatus.OK.value(), getLocalizedMessage("order.created.successfully", orderRequestDTO.getOrderId()), order);
    ...
    public ResponseEntity<ApiResponse<List<Order>>> getAllOrders(){
        ...
        ApiResponse<List<Order>> response = new ApiResponse<>(AppUtil.SUCCESS, HttpStatus.OK.value(), getLocalizedMessage("orders.fetched.successfully"), orders);
    ...
    public ResponseEntity<ApiResponse<Order>> getOrderById(@PathVariable Long orderId){
        ...
        if(order.isPresent()){
            ApiResponse<Order> response = new ApiResponse<>(AppUtil.SUCCESS,HttpStatus.OK.value(), getLocalizedMessage("order.fetched.successfully", orderId), order.get());
            return ResponseEntity.ok(response);
        }else {
            ApiResponse<Order> response = new ApiResponse<>(AppUtil.FAILURE, HttpStatus.NOT_FOUND.value(), getLocalizedMessage("order.not.found", orderId));
